{  
   "actor":{  
      "name":"Not stored in historic data",
      "account":{  
         "homePage":"http://moodle.data.alpha.jisc.ac.uk",
         "name":"3"
      }
   },
   "context":{  
      "platform":"Moodle",
      "language":"en",
      "extensions":{  
         "http://lrs.learninglocker.net/define/extensions/moodle_logstore_standard_log":{  
            "eventname":"\\mod_resource\\event\\course_module_viewed",
            "timecreated":"2016-01-19T12:00:18+00:00",
            "courseid":"3"
         },
         "http://lrs.learninglocker.net/define/extensions/info":"University of Jisc Moodle"
      },
      "contextActivities":{  
         "grouping":[  
            {  
               "id":"http://moodle.data.alpha.jisc.ac.uk/",
               "definition":{  
                  "type":"http://lrs.learninglocker.net/define/type/moodle/course_module",
                  "name":{  
                     "en":"University of Jisc Moodle"
                  },
                  "description":{  
                     "en":"Uni of Jisc Demo Moodle/"
                  }
               }
            },
            {  
               "id":"http://moodle.data.alpha.jisc.ac.uk/course_module/view.php?id=3",
               "definition":{  
                  "type":"http://lrs.learninglocker.net/define/type/moodle/course_module",
                  "name":{  
                     "en":"Not stored in historic data"
                  },
                  "description":{  
                     "en":"Not stored in historic data"
                  }
               }
            }
         ]
      }
   },
   "timestamp":"2016-01-19T12:00:18+00:00",
   "verb":{  
      "id":"http://id.tincanapi.com/verb/viewed",
      "display":{  
         "en":"viewed"
      }
   },
   "object":{  
      "id":"http://moodle.data.alpha.jisc.ac.uk/course_module/view.php?id=3",
      "definition":{  
         "type":"http://lrs.learninglocker.net/define/type/moodle/mod_resource",
         "name":{  
            "en":"35"
         },
         "description":{  
            "en":"A module"
         }
      }
   }
},